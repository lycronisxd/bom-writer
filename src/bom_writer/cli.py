...
"""))
        ...
        if path_conf == "Y" or path_conf == "y":
            ...
            if os.path.exists(f"{path}"):
                bom_filename = Path(path) / f"{name}-bom-{dmy}.csv"
                try:
...
""")
    ...
    with open(bom_filename, 'r') as bomcsv:
        ...
        rowadd_counter = 1
        while rowadd_counter <= extrarows:
...
""")
    ...
    newbom_name = Path(bom_filename).stem
    newbom_name = Path(md_filepath) / f"{newbom_name}-converted.md"
        
    try:
        open(newbom_name, 'x')
    except FileExistsError:
            print("file with same name already exists - retry.")
            mdconverter()
